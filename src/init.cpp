...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("H2O version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  H2Od [options]                     " + "\n" +
                  "  H2Od [options] <command> [params]  " + _("Send command to -server or H2Od") + "\n" +
                  "  H2Od [options] help                " + _("List commands") + "\n" +
                  "  H2Od [options] help <command>      " + _("Get help for a command") + "\n";
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: H2O.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: H2Od.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 9912 or testnet: 9913)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 9911 or testnet: 9911)") + "\n" +
        ...
        "\n" + _("SSL options: (see the H2O Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. H2O is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("H2O version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "H2O server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of H2O") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart H2O to complete") << "\n";
